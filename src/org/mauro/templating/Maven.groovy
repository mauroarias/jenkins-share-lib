...
class Maven implements Serializable {
    ...
    def public pushSonarArtifact (artifactId) {
        sh "mvn clean verify sonar:sonar -Dsonar.projectKey=${artifactId} -Dsonar.host.url=${Constants.getSonarHost()} -Dsonar.login=${steps.env.SONAR_TOKEN} -Dsonar.projectName=${artifactId}"
    }
}