...
def public applyGitRepository (gitDstRemote, serviceName, templateFullName, projectName) {
    ...
    GitRetriever.getGitInst().cloneRepo(this, branch, repoTemplate, templateName, serviceName)
    dir(serviceName) {
        sh "./prepare.sh ${serviceName}"
        sh "rm ./prepare.sh"
        GitRetriever.getGitInst().createRepo(this, serviceName, projectName)
        def pipelineFile = libraryResource 'org/mauro/templates/JenkinsfilePipelineJobWithLibTemplate'
        Tools.createJenkinsPipelineFileWithLib(this, pipelineFile, Config.getPipelineVersion(), templateName)
